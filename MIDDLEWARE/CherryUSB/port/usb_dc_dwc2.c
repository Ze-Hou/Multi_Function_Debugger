/*!
    \file       usb_dc_dwc2.c
    \brief      USB DWC2 device controller driver
    \version    1.0
    \date       2025-07-25
    \author     Ze-Hou
...
#define CONFIG_USB_DWC2_TX2_FIFO_SIZE (8 / 4)
#endif

#ifndef CONFIG_USB_DWC2_TX3_FIFO_SIZE
#define CONFIG_USB_DWC2_TX3_FIFO_SIZE (0 / 4)
#endif
...
static inline int dwc2_reset(uint8_t busid)
{
    ...
    while(USB_OTG_GLB->GRSTCTL & USB_OTG_GRSTCTL_CSRST);
...
{
    int ret;
    USB_OTG_GLB->GUSBCFG &= ~(USB_OTG_GUSBCFG_HSPHYSEL | USB_OTG_GUSBCFG_FSPHYSEL);
#if defined(CONFIG_USB_HS)
    /* Select FS Embedded PHY */
    USB_OTG_GLB->GUSBCFG |= USB_OTG_GUSBCFG_HSPHYSEL;

    /* Reset after a PHY select */
    ret = dwc2_reset(busid);
#else
    /* Select FS Embedded PHY */
    USB_OTG_GLB->GUSBCFG |= USB_OTG_GUSBCFG_FSPHYSEL;
...
static inline int dwc2_flush_rxfifo(uint8_t busid)
{
    ...
    while(USB_OTG_GLB->GRSTCTL & USB_OTG_GRSTCTL_RXFFLSH);
    return 0;
}
...
static inline int dwc2_flush_txfifo(uint8_t busid, uint32_t num)
{
    ...
    while(USB_OTG_GLB->GRSTCTL & USB_OTG_GRSTCTL_TXFFLSH);
    return 0;
}
...
static void dwc2_tx_fifo_empty_procecss(uint8_t busid, uint8_t ep_idx)
{
    ...
    if (g_dwc2_udc[busid].in_ep[ep_idx].xfer_len <= g_dwc2_udc[busid].in_ep[ep_idx].actual_xfer_len) {
        fifoemptymsk = (uint32_t)(0x1UL << (ep_idx & 0x7f));
...
int usb_dc_init(uint8_t busid)
{
    ...
    USB_OTG_DEV->DCTL |= USB_OTG_DCTL_SDIS;             /* �������Ͽ� */
    
    USB_OTG_GLB->GAHBCFG &= ~USB_OTG_GAHBCFG_GINT;
    
    ret = dwc2_core_init(busid);
    
    USB_OTG_GLB->GOTGCTL |= USB_OTG_GOTGCTL_BVALOVAL | USB_OTG_GOTGCTL_BVALOEN;

    /* This is vendor register */
    USB_OTG_GLB->GCCFG = usbd_get_dwc2_gccfg_conf(USBD_BASE);
    
    #ifdef CONFIG_USB_HS
        pllusb_rcu_config(USBD_BASE);
    #endif
...
int usbd_ep_start_write(uint8_t busid, const uint8_t ep, const uint8_t *data, uint32_t data_len)
{
    ...
    if (data_len > 0U) {
        USB_OTG_DEV->DIEPEMPMSK |= 1UL << (ep_idx & 0x7f);
    }
#endif
    return 0;
}
        ...
        if (gint_status & USB_OTG_GINTSTS_IEPINT) {
            ...
            while (ep_intr != 0U) {
                if ((ep_intr & 0x1U) != 0U) {
                    ...
                    #ifdef CONFIG_USB_DWC2_DMA_ENABLE
                        if(((USB_OTG_INEP(ep_idx)->DIEPTSIZ) & USB_OTG_DIEPTSIZ_XFRSIZ)<=USB_MAX_MPS)
                        {
                           (USB_OTG_INEP(ep_idx)->DIEPTSIZ) &=~ USB_OTG_DIEPTSIZ_XFRSIZ; 
                        }
                    #endif
